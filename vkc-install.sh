...
function stop_daemon {
    if pgrep -x 'vkcoind' > /dev/null; then
        echo -e "${YELLOW}Attempting to stop vkcoind${NC}"
        vkcoind-cli stop
        sleep 30
        if pgrep -x 'vkcoind' > /dev/null; then
            echo -e "${RED}vkcoind daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            sudo pkill -9 vkcoind
            sleep 30
            if pgrep -x 'vkcoind' > /dev/null; then
                echo -e "${RED}Can't stop vkcoind! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
sudo chmod -R 755 vkcoind
sudo cp -p -r vkcoind /usr/local/bin
sudo cp -p -r vkcoin-cli /usr/local/bin
 ...
 #Create datadir
 if [ ! -f ~/.vkcoin/vkcoin.conf ]; then 
 	sudo mkdir ~/.vkcoin
 fi
clear
echo -e "${YELLOW}Creating vkcoin.conf...${NC}"

# If genkey was not supplied in command line, we will generate private key on the fly
if [ -z $genkey ]; then
    cat <<EOF > ~/.vkcoin/vkcoin.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
EOF

    sudo chmod 755 -R ~/.vkcoin/vkcoin.conf
...
#Finally, starting daemon with new vkcoin.conf
vkcoind -daemon
sleep 5

#Setting auto start cron job for vkcoin
cronjob="@reboot sleep 30 && vkcoin"
crontab -l > tempcron
...
======================================================================== \a"
echo -e "${GREEN}vkcoin_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it 
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}vkcoin_mn1${NC} - with your desired masternode name (alias)
    ${GREEN}TxId${NC} - with Transaction Id from masternode outputs
    ${GREEN}TxIdx${NC} - with Transaction Index (0 or 1)
     Remember to save the masternode.conf and restart the wallet!
To introduce your new masternode to the vkcoin network, you need to
...
========================================================================
To view masternode configuration produced by this script in vkcoin.conf:
${GREEN}cat ~/.vkcoin/vkcoin.conf${NC}
Here is your vkcoin.conf generated by this script:
-------------------------------------------------${GREEN}"
echo -e "${GREEN}vkcoin_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
cat ~/.vkcoin/vkcoin.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit vkcoin.conf, first stop the vkcoind daemon,
then edit the vkcoin.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the vkcoind daemon back up:
