...
{
    // return if URI is not valid or is no vkcoin: URI
...
{
    // Convert vkcoin:// to vkcoin:
    //
    //    Cannot handle this later, because vkcoin:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "vkcoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for vkcoin.lnk
...
{
    return GetAutostartDir() / "vkcoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a vkcoin.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the vkcoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add vkcoin app to startup item list
