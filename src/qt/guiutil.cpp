...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a VKC address (e.g. %1)").arg("eSBxEjGXmJxwpXxXAJqeXztdUR4MsZZJsE"));
#endif
...
{
    // return if URI is not valid or is no vkccoin: URI
...
{
    // Convert vkccoin:// to vkccoin:
    //
    //    Cannot handle this later, because vkccoin:// will cause Qt to see the part after // as host,
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "vkccoin.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for vkccoin.lnk
...
{
    return GetAutostartDir() / "vkccoin.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a vkccoin.desktop file to the autostart directory:
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the vkccoin app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add vkccoin app to startup item list
