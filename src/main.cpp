...
#if defined(NDEBUG)
#error "vkccoin cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvkccoin) are considered zero fee (for relaying and mining)
...
bool CVerifyDB::VerifyDB(CCoinsView* coinsview, int nCheckLevel, int nCheckDepth)
{
    ...
    for (CBlockIndex* pindex = chainActive.Tip(); pindex && pindex->pprev; pindex = pindex->pprev) {
        ...
        if (!ReadBlockFromDisk(block, pindex))
            return error("VerifyDB(0) : *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
        // check level 1: verify block validity
        if (nCheckLevel >= 1 && !CheckBlock(block, state))
            return error("VerifyDB(1) : *** found bad block at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
        ...
        if (nCheckLevel >= 2 && pindex) {
            ...
            if (!pos.IsNull()) {
                if (!undo.ReadFromDisk(pos, pindex->pprev->GetBlockHash()))
                    return error("VerifyDB(2) : *** found bad undo data at %d, hash=%s\n", pindex->nHeight, pindex->GetBlockHash().ToString());
            }
        }
        ...
        if (nCheckLevel >= 3 && pindex == pindexState && (coins.GetCacheSize() + pcoinsTip->GetCacheSize()) <= nCoinCacheSize) {
            ...
            if (!DisconnectBlock(block, state, pindex, coins, &fClean))
                return error("VerifyDB(3) : *** irrecoverable inconsistency in block data at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
    ...
    if (nCheckLevel >= 4) {
        ...
        while (pindex != chainActive.Tip()) {
            ...
            CBlock block;
            if (!ReadBlockFromDisk(block, pindex))
                return error("VerifyDB(4) : *** ReadBlockFromDisk failed at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
            if (!ConnectBlock(block, state, pindex, coins, false))
                return error("VerifyDB(4) : *** found unconnectable block at %d, hash=%s", pindex->nHeight, pindex->GetBlockHash().ToString());
        }
    }
