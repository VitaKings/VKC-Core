...
function stop_daemon {
    if pgrep -x 'vkccoind' > /dev/null; then
        ...
        sleep 30
        if pgrep -x 'vkccoind' > /dev/null; then
            echo -e "${RED}vkccoind daemon is still running!${NC} \a"
            echo -e "${RED}Attempting to kill...${NC}"
            sudo pkill -9 vkccoind
            sleep 30
            if pgrep -x 'vkccoind' > /dev/null; then
                echo -e "${RED}Can't stop vkccoind! Reboot and try again...${NC} \a"
                exit 2
            fi
        fi
    fi
...
echo -e "${GREEN} ------- e-Sport Betting Coin MASTERNODE INSTALLER v2.1.0--------+
 ...
 |        the masternode under a user VKC.         |::
 |                                                  |::
...
sudo cp -p -r vkccoin-cli /usr/local/bin
 ...
 #Create datadir
 if [ ! -f ~/.vkccoin/vkccoin.conf ]; then 
 	sudo mkdir ~/.vkccoin
 fi
clear
echo -e "${YELLOW}Creating vkccoin.conf...${NC}"

# If genkey was not supplied in command line, we will generate private key on the fly
if [ -z $genkey ]; then
    cat <<EOF > ~/.vkccoin/vkccoin.conf
rpcuser=$rpcuser
rpcpassword=$rpcpassword
EOF

    sudo chmod 755 -R ~/.vkccoin/vkccoin.conf
...
echo -ne '\n'
    ...
    #Stopping daemon to create vkccoin.conf
    vkccoin-cli stop
    sleep .5
fi
# Create vkccoin.conf
...
#Finally, starting daemon with new vkccoin.conf
vkccoind -daemon
sleep 5

#Setting auto start cron job for vkccoin
cronjob="@reboot sleep 30 && vkccoin"
crontab -l > tempcron
...
======================================================================== \a"
echo -e "${GREEN}vkccoin_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
echo -e "========================================================================
Use your mouse to copy the whole string above into the clipboard by
tripple-click + single-click (Dont use Ctrl-C) and then paste it 
into your ${GREEN}masternode.conf${NC} file and replace:
    ${GREEN}vkccoin_mn1${NC} - with your desired masternode name (alias)
    ${GREEN}TxId${NC} - with Transaction Id from masternode outputs
    ${GREEN}TxIdx${NC} - with Transaction Index (0 or 1)
     Remember to save the masternode.conf and restart the wallet!
To introduce your new masternode to the vkccoin network, you need to
...
========================================================================
To view masternode configuration produced by this script in vkccoin.conf:
${GREEN}cat ~/.vkccoin/vkccoin.conf${NC}
Here is your vkccoin.conf generated by this script:
-------------------------------------------------${GREEN}"
echo -e "${GREEN}vkccoin_mn1 $publicip:$PORT $genkey TxId TxIdx${NC}"
cat ~/.vkccoin/vkccoin.conf
echo -e "${NC}-------------------------------------------------
NOTE: To edit vkccoin.conf, first stop the vkccoind daemon,
then edit the vkccoin.conf file and save it in nano: (Ctrl-X + Y + Enter),
then start the vkccoind daemon back up:
to stop:              ${GREEN}vkccoin-cli stop${NC}
to start:             ${GREEN}vkccoind${NC}
to edit:              ${GREEN}nano ~/.vkccoin/vkccoin.conf${NC}
